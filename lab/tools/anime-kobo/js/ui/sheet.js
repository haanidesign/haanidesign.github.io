...
import { S, onChange, beginEdit, commitEdit, edit, selected, addAsset, plain } from '../state.js?v=258';
import { isDescendant, setParent, isFolder, membersOf, ungroup, mergeAsFrames,
         attachMany, copyLayers, pasteLayers, removeLayers,
         duplicateLayers, newPaintLayer, newSolidLayer,
         newFlip, isFlip, flipIndex, groupInto,
         splitFrames, newCamLayer, nearestFolder } from '../engine/layer.js?v=258';
import { hasPins, setPin, channelValue, valuesAt, spreadFrames,
         framePinTimes, removePin, pinChX, pinChY, EASES, EASE_LIST,
         curveAt, MY_EASE_MAX } from '../engine/anim.js?v=258';
import { swayKeys, swayPose, newSway, RIGID,
         afterKeys, afterAngle, afterLen, stopTimes } from '../engine/puppet.js?v=258';
import { pathKeys, pathLength, resample } from '../engine/path.js?v=258';
import { blinkKeys, talkKeys } from '../engine/anim.js?v=258';
import { PRESET_GROUPS, CATS } from '../engine/presets.js?v=258';
import { FONTS, renderTextLayer, shortName, newTextStyle, textToCanvas,
         addTextLayer } from '../io/text.js?v=258';
import { addBgLayer, paintBg, fitToCanvas, isBg,
         paintPattern, addPatternBg, DIR_PRESETS } from '../io/bg.js?v=258';
import { PATTERN_NAMES } from '../io/pattern.js?v=258';
import { isPano, addPanoLayer, spinKeys, sweepKeys, panoDefaults,
         PITCH_MAX } from '../engine/pano.js?v=258';
import { ballOn, ballDefaults, ballSpinKeys } from '../engine/ball.js?v=258';
import { isRoom, addRoomLayer, FACES as ROOM_FACES } from '../engine/room.js?v=258';
import { isTalk, addTalkLayer, addNextTalk, talkDefaults, talkMouthKeys,
         talkEnd, talkStart, talkOut, niceHold,
         overlapping, fixOverlaps } from '../engine/talk.js?v=258';
import { readAsDataURL, loadImage } from '../io/image.js?v=258';
import { isCam, camOf, resetCam, depthScale, is3D, ORBIT_MAX,
         DOLLY_MIN, DOLLY_MAX, depthOf, CAM_CHANNELS,
         DEPTH_MIN, DEPTH_MAX, DEPTH_PRESETS } from '../engine/camera.js?v=258';
import { bakeLayers, applyBake } from '../io/flatten.js?v=258';
import { newHand } from '../engine/hand.js?v=258';
import { newReveal, totalLen, paintDirty } from '../engine/paint.js?v=258';
import { createWheel, favs, addFav, delFav, hasFav, parseHex, hex as toHex }
  from './colorwheel.js?v=258';
import { A as AUD, hasAudio, clearAudio, voiceMouthKeys, speechSpans, levels,
         startRec, stopRec, cancelRec, isRecording, setPitch,
         guessBpm, firstOnset, playBlip } from '../io/audio.js?v=258';
import { rhythmKeys, rhythmChannels, beatTimes, beatSec, markKeys,
         RHYTHM_KINDS, putHit } from '../engine/rhythm.js?v=258';
...
/* ---------- 見出しごと たたむ ----------
   せっていは 見出しが 10こ、たかさ 1664px あって、
   せつめいを 1行に しても ボタンの 数は へらない ので
   さがして スクロールする のは 変わらなかった。
   見出しを おしたら その まとまりだけ 開く ように する。
   とじて いれば 見出しだけ ＝ 目次に なる。

   開け閉めは おぼえて おく。シートは 何か さわる たびに
   まるごと 作り直される ので、DOM に おいて おけない。 */
const secOpen = new Map();

function foldSections(host){
  const heads = [...host.children].filter(e => e.tagName === 'H2' && e.dataset.sec);
  /* 見出しが 1つ しか 無い 画面（ちいさな ページ）は たたまない。
     ぜんぶ 消えて「こわれた？」に なる ため。 */
  ...
  heads.forEach(h => {
    ...
    /* つぎの 見出しまでを 1つの ふくろに 入れる */
    const body = document.createElement('div');
    let n = h.nextSibling;
    while(n && !(n.nodeType === 1 && n.tagName === 'H2' && n.dataset.sec)){
    ...
    mark.style.cssText = 'display:inline-block;width:1em;opacity:.6';
    h.insertBefore(mark, h.firstChild);

    const show = () => {
      const open = secOpen.get(key) === true;
      body.hidden = !open;
    ...
    h.addEventListener('click', () => {
      secOpen.set(key, secOpen.get(key) !== true);
      show();
    });
  });
}
...
export function heading(text){
  ...
  h.textContent = text;
  /* あとで まとまりごと たたむ ときの 目じるし と 合いことば。
     シートの 題名も h2 なので、これが ある ものだけ たたむ。
     中みを 作り直しても 開け閉めを おぼえて おける ように、
     おぼえる かぎは 「見出しの ことば」に する
     （？を あとから 足すので textContent は つかえない）。 */
  h.dataset.sec = text;
  return h;
}
