...
import { S, onChange, beginEdit, commitEdit, edit, selected, addAsset, plain } from '../state.js?v=296';
import { isDescendant, setParent, isFolder, membersOf, ungroup, mergeAsFrames,
         ...
         splitFrames, newCamLayer, nearestFolder } from '../engine/layer.js?v=296';
import { masksOf, toMasks, maskAnimated, clearMaskKeys, setMaskKeys } from '../engine/mask.js?v=296';
import { hasPins, setPin, channelValue, valuesAt, spreadFrames,
         framePinTimes, removePin, pinChX, pinChY, EASES, EASE_LIST,
         curveAt, MY_EASE_MAX } from '../engine/anim.js?v=296';
import { swayKeys, swayPose, newSway, RIGID,
         afterKeys, afterAngle, afterLen, stopTimes } from '../engine/puppet.js?v=296';
import { pathKeys, pathLength, resample } from '../engine/path.js?v=296';
import { blinkKeys, talkKeys } from '../engine/anim.js?v=296';
import { PRESET_GROUPS, CATS } from '../engine/presets.js?v=296';
import { applyRig, rigRootOf, newRigSet } from '../io/rig.js?v=296';
import { addLinesLayer, newLines, isLines } from '../io/lines.js?v=296';
import { FONTS, renderTextLayer, shortName, newTextStyle, textToCanvas,
         addTextLayer } from '../io/text.js?v=296';
import { addBgLayer, paintBg, fitToCanvas, isBg,
         paintPattern, addPatternBg, DIR_PRESETS } from '../io/bg.js?v=296';
import { PATTERN_NAMES } from '../io/pattern.js?v=296';
import { isPano, addPanoLayer, spinKeys, sweepKeys, panoDefaults,
         PITCH_MAX } from '../engine/pano.js?v=296';
import { ballOn, ballDefaults, ballSpinKeys } from '../engine/ball.js?v=296';
import { isRoom, addRoomLayer, FACES as ROOM_FACES } from '../engine/room.js?v=296';
import { isTalk, addTalkLayer, addNextTalk, talkDefaults, talkMouthKeys,
         talkEnd, talkStart, talkOut, niceHold,
         overlapping, fixOverlaps } from '../engine/talk.js?v=296';
import { readAsDataURL, loadImage } from '../io/image.js?v=296';
import { isCam, camOf, resetCam, depthScale, is3D, ORBIT_MAX,
         DOLLY_MIN, DOLLY_MAX, depthOf, CAM_CHANNELS,
         DEPTH_MIN, DEPTH_MAX, DEPTH_PRESETS } from '../engine/camera.js?v=296';
import { bakeLayers, applyBake } from '../io/flatten.js?v=296';
import { newHand } from '../engine/hand.js?v=296';
import { newReveal, totalLen, paintDirty } from '../engine/paint.js?v=296';
import { createWheel, favs, addFav, delFav, hasFav, parseHex, hex as toHex }
  from './colorwheel.js?v=296';
import { A as AUD, hasAudio, clearAudio, voiceMouthKeys, speechSpans, levels,
         startRec, stopRec, cancelRec, isRecording, setPitch,
         guessBpm, firstOnset, playBlip } from '../io/audio.js?v=296';
import { rhythmKeys, rhythmChannels, beatTimes, beatSec, markKeys,
         RHYTHM_KINDS, putHit } from '../engine/rhythm.js?v=296';
...
export function buildLayerSheet(box, closeFn){
  ...
  if(l.kind !== 'adjust'){
    box.appendChild(field('トラックマット', (() => {
      ...
      [['', 'つかわない'],
       ['alpha',    '上の 形で ぬく'],
       ['alphaInv', '上の 形を 消す（ぎゃく）'],
       ['luma',     '上の 明るい ところを のこす'],
       ['lumaInv',  '上の 暗い ところを のこす']
      ].forEach(([v2, label]) => {
        const o = document.createElement('option');
        o.value = v2; o.textContent = label;
        if((l.matte || '') === v2) o.selected = true;
        sel.appendChild(o);
      });
  ...
  if(l.kind === 'adjust'){
    ...
    n.className = 'empty';
    n.style.textAlign = 'left';
    n.textContent = 'この かみ じたいは 見えません。' + NL
      + '下に ある ものへ まとめて 色の 調整・ぼかしを かけます。' + NL
      + '四角を 小さく すると、その ところ だけに かかります。' + NL
      + '（✂ マスクを かこめば その 形の 中だけ）';
    box.appendChild(n);
    box.appendChild(clipRow(l));
    spanRow(box, l, closeFn);
...
function linesRow(box, closeFn){
  ...
  const o = Object.assign(newLines(), editing ? editing.lines : {});
  ...
  kindSel.addEventListener('change', () => { o.kind = kindSel.value; });
  ...
  box.appendChild(btnRow(
    button(editing ? '💥 作り直す' : '💥 入れる', async () => {
      ...
      commitEdit();
      notify(editing ? '作り直しました' : '入れました（重ねる 順番は レイヤーで）');
      onChange();
      if(closeFn) closeFn();
    })
  ));
...
export function buildLook(box, l, opts){
  ...
  mnote.className = 'empty';
  mnote.style.textAlign = 'left';
  mnote.textContent = ms.length
    ? (ms.length + 'まい かかって います。' + NL
       + (isFolder(l)
          ? 'フォルダの マスクは 画面に すわった まま です。'
          : 'マスクは 絵に くっついて いるので、動かしても ついて まわります。'))
    : ('絵の 上を 指で ぐるっと かこむと、その 中だけ 出ます。' + NL
       + 'かこんだ 形は ' + (isFolder(l) ? '画面に すわります。' : '絵に くっつきます。') + NL
       + '何まいでも 足せます（たす／ぬく）。');
  box.appendChild(mnote);
  ...
  ms.forEach((m, mi) => {
    ...
    wrap.textContent = (mi + 1) + 'まいめ … ' + (m.mode === 'sub' ? 'ぬく' : 'たす')
      + (m.invert ? '（うら返し）' : '')
      + '・点 ' + (m.pts ? m.pts.length : 0) + 'コ';
    box.appendChild(wrap);

    box.appendChild(btnRow(
      button(m.mode === 'sub' ? '➖ ぬく' : '➕ たす', () => {
        edit('マスクの きき方', () => {
          const list = toMasks(l);
          list[mi].mode = list[mi].mode === 'sub' ? 'add' : 'sub';
          l._maskKey = null;
        });
        onChange();
        if(closeLook) closeLook();
      ...
      button(m.invert ? '✅ うら返す' : '⬜ うら返す', () => {
        edit('マスクを うら返す', () => {
          const list = toMasks(l);
          list[mi].invert = !list[mi].invert;
          l._maskKey = null;
        });
        onChange();
        if(closeLook) closeLook();
    ...
    box.appendChild(slider('ふちを ぼかす',
      () => (m.feather || 0),
      v => { const list = toMasks(l); list[mi].feather = v; l._maskKey = null; },
  ...
  box.appendChild(heading('📌 カメラに 合わせない もの'));
  {
    ...
    if(!rows.length){
      ...
      rows.forEach(({ x, depth }) => {
        ...
        const show = () => {
          /* なまえ だけ だと、どちらの 状態なのか 分からない と
             言われた。いま どう なって いるかを 字で 書く。 */
          b.textContent = pad + (isFolder(x) ? '📁 ' : '') + (x.name || 'レイヤー')
            + '　… ' + (x.noCam ? '📌 合わせない' : '🎥 合わせる');
...
export function buildExportSheet(box, closeFn, run){
  ...
  box.appendChild(heading('🎬 アフターエフェクトへ'));
  const ae = document.createElement('div');
  ae.className = 'empty';
  ae.style.textAlign = 'left';
  const aeN = Math.round(S.proj.duration * (S.proj.fps || 30));
  const aeL = S.proj.layers.filter(l => l.kind !== 'cam' && l.kind !== 'audio'
                                     && l.kind !== 'folder' && l.visible !== false).length;
  // 1コマ だいたい 0.2MB（実測の めやす）
  ...
  ae.textContent = 'レイヤーを 1まいずつ 連番の 絵に 焼いて、'
    ...
    + NL + '時間も ファイルも 大きく なるので PCで どうぞ。'
    + NL + 'zipは ぜんぶ 展開してから .jsx を AEで 実行します。';
  box.appendChild(ae);
  box.appendChild(btnRow(
    button('🎬 AE用に 書き出す（zip）', () => { if(closeFn) closeFn(); run('ae'); })
  ));
}
...
export function buildPaintSheet(box, closeFn){
  ...
  box.appendChild(btnRow(
    ...
    button('🎚 ちょうせいの かみ', () => {
      const made = {};
      edit('ちょうせいの かみ', () => {
        made.l = newAdjustLayer('ちょうせい', S.proj.w, S.proj.h);
        S.proj.layers.unshift(made.l);
        S.sel = made.l.id;
      });
      notify('下の ものに まとめて 効果を かける かみです');
      onChange();
      if(closeFn) closeFn();
    }),
    button('🟪 いろの かみ', () => {
...
export function remapRow(box, l){
  ...
  const sw = button('', () => {
    edit('時間いじり', () => {
      if(on()){ l.remap = false; }
        ...
        l.remap = true;
        if(!l.tracks || !l.tracks.time || !l.tracks.time.length){
          // はじめは そのまま（1秒＝1秒）
          setPin(l, 'time', 0, 0, 'linear');
          setPin(l, 'time', dur, dur, 'linear');
        }
      }
    });
    notify(on() ? 'この レイヤーの 時間を いじれます' : '時間いじりを やめました');
    onChange();
    openLayer();
  });
  sw.textContent = (on() ? '✅' : '⬜') + ' 時間を いじる';
  sw.classList.toggle('on', on());
  box.appendChild(btnRow(sw));
  ...
  /* いまの 時こくで 見せて いる「中みの 時こく」 */
  const now = () => {
    const k = (l.tracks || {}).time;
    return k && k.length ? channelValue(l, 'time', S.time) : S.time;
  };
  const sl = document.createElement('input');
  sl.type = 'range'; sl.min = 0; sl.max = dur; sl.step = 0.01; sl.value = now();
  const val = document.createElement('span');
  val.className = 'val';
  const show = () => { val.textContent = (+sl.value).toFixed(2) + '秒めを 見せる'; };
  show();
  sl.addEventListener('pointerdown', () => { holdSheet(true); beginEdit('見せる 時こく'); });
  guardSlide(sl, () => {
    setPin(l, 'time', S.time, +sl.value, 'linear');
    show();
    onChange();
  });
  sl.addEventListener('change', () => { holdSheet(false); commitEdit(); });
  box.appendChild(field('いま 見せる ところ', sl, val));

  const setKeys = (label, pairs, msg) => button(label, () => {
    edit(label, () => {
      if(l.tracks) delete l.tracks.time;
      pairs.forEach(([t, v]) => setPin(l, 'time', t, v, 'linear'));
    });
    notify(msg);
    onChange();
    openLayer();
  ...
  box.appendChild(btnRow(
    setKeys('⏸ ここから 止める', [[0, 0], [S.time, S.time], [dur, S.time]],
            'この あと 絵が 止まります'),
    setKeys('↩ ぎゃく回し', [[0, dur], [dur, 0]], 'うしろから 流れます')
  ));
  box.appendChild(btnRow(
    setKeys('🐢 半分の はやさ', [[0, 0], [dur, dur / 2]], 'ゆっくりに なります'),
    setKeys('🐇 2ばいの はやさ', [[0, 0], [dur, dur * 2]], 'はやくなります'),
    setKeys('もとに もどす', [[0, 0], [dur, dur]], 'そのままの はやさに もどしました')
  ));
