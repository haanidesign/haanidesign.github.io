...
import { M, uid, ptInQuad } from './math.js?v=281';
import { valuesAt as evalAt, setPin, shiftTrack, remapTime } from './anim.js?v=281';
import { isCam, camOf, camMatrix, depthLen, is3D, quad3D,
         camOrbiting, sheetQuad3D, quadFromM, camDefocus,
         withShake } from './camera.js?v=281';
import { deformPoint, swayPose, swayTilt } from './puppet.js?v=281';
import { cageDeformPoint, cageMoved, homography, applyH } from './warp.js?v=281';
import { handTime } from './hand.js?v=281';
import { WORK_KEYS } from '../state.js?v=281';
...
export function newCamLayer(project){
  ...
  l.lockAspect = true;
  l.z = 0;                       // ドリー（前後に 動く）
  l.aim = false;                 // 注視点を つかうか
  l.tx = project.w / 2;          // 注視点
  l.ty = project.h / 2;
  l.td = 0;
  l.fd = 0;                      // ピントの おくゆき
  l.dof = 0;                     // ピンぼけの つよさ
...
/** 音の 行。絵は 出さない。タイムラインに 波形を 出す ための もの。
    音そのものは 1つしか 持たない ので、この 行も 1つだけ。 */
export function newAudioLayer(name){
  const l = newLayer(name || 'おと', []);
  l.kind = 'audio';
  l.locked = true;        // 絵の上では さわれない
  return l;
}

export const isAudioLayer = (l) => !!l && l.kind === 'audio';
...
export function computeAll(project, time){
  ...
  const camV = cam ? withShake(evalAt(cam, time), cam, time, project) : null;
  ...
  const solve = (l) => {
    ...
    /* カメラを だれが 受けとるか。
       ...
       ふつうは 親の いない レイヤーが 1回だけ 受けとり、
       子は 親の 姿ごしに ついてくる（二重に かからない）。

       「バラで 動かす」（AEの コラップス）を 入れた フォルダは
       自分では 受けとらず、中身に そのまま 通す。
       すると 中身の 1まい 1まいが 自分の おくゆきで 動く
