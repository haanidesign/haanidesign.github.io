...
export function drawDeformed(ctx, img, mesh, xy, srcK, uv, mode){
  ...
  /* ---------- 三角の つぎ目を 出さない ----------
     ...
     少し ふくらませて 上から ぬる。
     かさなっても 同じ 中みを ぬり直すだけ なので、
     形が 動いて いても ちらつかない。

     ＊ 足し算（lighter）で つなぐ やり方も ためした。
       つぎ目は ぴたりと 消える が、あみが 動く もの
       （手がき風・ゆがみ・ピン）では 三角が わずかに かさなった
       ところで 足しすぎに なり、線の 太さが コマごとに 変わって
       チカチカした。動く ものの ほうが だいじ なので こちらに もどす。
